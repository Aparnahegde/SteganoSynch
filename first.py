...
def showImage():
    ...
    if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
        messagebox.showinfo("", "Image file Selected Successfully!!")
           
        #openImage(filename)
    elif filename.lower().endswith('.mp4'):
        #openVideo()
        messagebox.showinfo("", "Video file Selected Successfully!!")
    elif filename.lower().endswith('.mov'):
        #openVideo()
        messagebox.showinfo("", "Video file Selected Successfully!!")
    elif filename.lower().endswith('.wav'):
        #open_audio(filename)
        messagebox.showinfo("", "Audio file Selected Successfully!!")
...
def Hide():
    ...
    n=len(secret_key)
    if n!=5 and n!=0:
           messagebox.showerror("Error", "Length of the secrete key should be 5!")
           return
    if not secret_key:
        messagebox.showwarning("Warning", "Please provide a secret key")
        return
        ...
        for i in range(len(frame_bytes)):
            ...
            for byte in all_bytes:
                ...
                if decoded_data[-5:] == "*^*^*": 
                   ...
                   key2=secret_key
                   if not secret_key:
                      messagebox.showwarning("Error", "Please provide a secret key")
                      return
                   #secret_key_bytes = bytes(secret_key, 'utf-8')
...
#icon
#secrete key
entry_secret_key = Entry(root, show="*", font="robote 20", bg="gray19", fg="white", relief=GROOVE)
entry_secret_key.place(x=310, y=485, width=360, height=40)
Label(root,text="Enter Secrete Key Here:",bg="gray10",fg="snow",font="Georgia 14 bold underline").place(x=30,y=495)

#Head
Label(root,text="StyganoSynch",bg="gray10",fg="white",font="Georgia 25 bold underline").place(x=200,y=20)
...
#second Frame
frame2=Frame(root,bd=3,width=680,height=280,bg="gray",relief=GROOVE)
frame2.place(x=10,y=80)

input_string=Text(frame2,font="robote 20",bg="gray19",fg="white",relief=GROOVE,wrap=WORD)
input_string.place(x=0,y=0,width=680,height=295)

scrollbar1=Scrollbar(frame2,bg="gray19")
scrollbar1.place(x=665,y=0,height=300)

scrollbar1.configure(command=input_string.yview)
input_string.configure(yscrollcommand=scrollbar1.set)

#third Frame
frame3=Frame(root,bd=3,bg="gray19",width=680,height=100,relief=GROOVE)
frame3.place(x=10,y=370)

Button(frame3,text="Open Media",bg="RoyalBlue1",width=10,height=3,font="Georgia 10 bold",command=showImage).place(x=20,y=20)
Button(frame3,text="Save Media",bg="RoyalBlue1",width=10,height=3,font="Georgia 10 bold",command=Save).place(x=150,y=20)
Button(frame3,text="Hide Data",bg="RoyalBlue1",width=10,height=3,font="Georgia 10 bold",command=Hide).place(x=280,y=20)
Button(frame3,text="Show Data",bg="RoyalBlue1",width=10,height=3,font="Georgia 10 bold",command=Show).place(x=415,y=20)
Button(frame3,text="Share Media",bg="RoyalBlue1",width=10,height=3,font="Georgia 10 bold",command=lambda: webbrowser.open("https://mail.google.com/")).place(x=550,y=20)
